...
struct VmRuntimeStats {
    ...
    size_t platform_pending_foreground_tasks = 0;
    size_t platform_pending_background_tasks = 0;
    size_t platform_delayed_task_count = 0;
    uint64_t platform_pump_count = 0;
    uint64_t platform_pump_iterations = 0;
    double platform_average_foreground_wait_ms = 0.0;
    double platform_average_background_wait_ms = 0.0;
    double platform_last_drain_ms = 0.0;
    double platform_overload_score = 0.0;
...
public:
    ...
    size_t creator_thread_hash() const { return creator_thread_hash_; }
    size_t vm_thread_hash() const { return vm_thread_hash_.load(std::memory_order_acquire); }
...
private:
    ...
    size_t stale_timer_queue_count_ = 0;
    ...
    void ReleaseTimerHandles(const std::shared_ptr<TimerTask>& task);
    void MaybeCompactTimerQueue();
    void CompactTimerQueue();
    bool TryPostTask(Task task, bool allow_while_disposing);

    // VM 线程管理
    void StartVmThread();
    void StopVmThread();
    void ThreadMain();
    void RefreshPlatformRuntimeStats(VmRuntimeStats& stats) const;
    bool QuiescePlatformForShutdown(std::chrono::milliseconds timeout,
                                    size_t max_iterations_per_drain = 256,
                                    size_t consecutive_idle_rounds = 3);

    // VM 线程相关成员
    std::thread vm_thread_;
    std::atomic<size_t> vm_thread_hash_{0};
    std::mutex task_mu_;
