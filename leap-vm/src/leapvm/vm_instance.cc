...
bool ShouldTraceVmThreadAffinity() {
#if defined(__linux__)
    const char* raw = std::getenv("LEAPVM_TRACE_VM_THREAD_AFFINITY");
    if (!raw || raw[0] == '\0') {
        return false;
    }
    std::string v(raw);
...
bool ShouldRunShutdownPlatformOnVmThread() {
#if defined(__linux__)
    const char* raw = std::getenv("LEAPVM_SHUTDOWN_PLATFORM_ON_VM_THREAD");
    if (!raw || raw[0] == '\0') {
        return false;
    }
    std::string v(raw);
...
bool ShouldPrepareShutdownAfterVmThread() {
#if defined(__linux__)
    const char* raw = std::getenv("LEAPVM_PREPARE_SHUTDOWN_AFTER_VM_THREAD");
    if (!raw || raw[0] == '\0') {
        return false;
    }
    std::string v(raw);
...
VmInstance::VmInstance() {
    ...
    creator_thread_hash_ = CurrentThreadHash();
    LEAPVM_LOG_INFO("Creating VmInstance...");
    if (ShouldTraceVmThreadAffinity()) {
        std::fprintf(stderr,
                     "[leapvm][vm] ctor vm=%p creator_tid=%zu\n",
                     static_cast<void*>(this),
                     creator_thread_hash_);
        std::fflush(stderr);
    }
...
VmInstance::~VmInstance() {
    const size_t destroy_thread_hash = CurrentThreadHash();
    const bool destroying_on_creator_thread =
        std::this_thread::get_id() == creator_thread_id_;
    if (ShouldTraceVmThreadAffinity() || !destroying_on_creator_thread) {
        std::fprintf(stderr,
                     "[leapvm][vm] dtor vm=%p destroy_tid=%zu creator_tid=%zu vm_thread_tid=%zu same_as_creator=%d\n",
                     static_cast<void*>(this),
                     destroy_thread_hash,
                     creator_thread_hash_,
                     vm_thread_hash(),
                     destroying_on_creator_thread ? 1 : 0);
        std::fflush(stderr);
    }
    if (!destroying_on_creator_thread) {
        LEAPVM_LOG_WARN("[shutdown] VmInstance destroyed off creator thread destroy_tid=%zu creator_tid=%zu vm_thread_tid=%zu",
                        destroy_thread_hash,
                        creator_thread_hash_,
                        vm_thread_hash());
    }
    ...
    if (inspector_client_ && vm_thread_running_) {
        ...
        if (PostTask([this, &inspector_done](v8::Isolate* isolate, v8::Local<v8::Context> context) {
            (void)isolate;
    ...
    const bool prepare_shutdown_after_vm_thread =
        ShouldPrepareShutdownAfterVmThread();
    bool platform_shutdown_on_vm_thread_done = false;
    if (isolate_ && vm_thread_running_ && ShouldRunShutdownPlatformOnVmThread()) {
        std::promise<void> platform_shutdown_done;
        auto platform_shutdown_done_future = platform_shutdown_done.get_future();
        if (PostTask([this, &platform_shutdown_done](v8::Isolate* isolate,
                                                    v8::Local<v8::Context> context) {
            (void)context;
            V8Platform::Instance().PrepareIsolateForShutdown(isolate);
            isolate->LowMemoryNotification();
            QuiescePlatformForShutdown(std::chrono::milliseconds(50));
            platform_shutdown_done.set_value();
        })) {
            platform_shutdown_done_future.wait();
            platform_shutdown_on_vm_thread_done = true;
            LEAPVM_LOG_INFO("[shutdown] platform shutdown prelude ran on VM thread");
        }
    }
    if (isolate_ && !platform_shutdown_on_vm_thread_done &&
        !prepare_shutdown_after_vm_thread) {
        V8Platform::Instance().PrepareIsolateForShutdown(isolate_);
    }
    {
    ...
    if (isolate_) {
        // 通知 DefaultPlatform 清理内部 per-isolate 状态（foreground_task_runner_map_ 等）。
        // 必须在 isolate_->Dispose() 之前调用，否则 DefaultPlatform 会累积 stale 条目，
        // 当新 isolate 被 malloc 分配到相同地址时，会命中旧的 stale runner 导致 SIGSEGV。
        auto* backend = V8Platform::Instance().backend_platform();
        if (backend) {
            v8::platform::NotifyIsolateShutdown(backend, isolate_);
        }
...
void VmInstance::InstallBuiltinWrappers(BuiltinWrapperConfig config) {
    ...
    if (!PostTask([this, config = std::move(config), &done_promise]
...
bool VmInstance::RunScript(const std::string& source_utf8,
    ...
    // 将执行逻辑包装成任务投递到 VM 线程
    if (!PostTask([this, source_utf8, resource_name, &result_out, error_out, &done_promise]
        ...
        if (error_out) {
            *error_out = "VmInstance is not accepting new tasks";
        }
        return false;
    }
...
bool VmInstance::CreateCodeCache(const std::string& source_utf8,
    ...
    if (!PostTask([this, source_utf8, resource_name, &cache_out, error_out, &done_promise]
                  (v8::Isolate* isolate, v8::Local<v8::Context> context) {
        v8::HandleScope handle_scope(isolate);
        v8::Context::Scope context_scope(context);
...
bool VmInstance::RunScriptWithCache(const std::string& source_utf8,
    ...
    if (!PostTask([this, source_utf8, resource_name, cache_copy = std::move(cache_copy),
...
bool VmInstance::ClearTimer(uint64_t id) {
    ...
    ReleaseTimerHandles(task);
    timers_by_id_.erase(it);
    stale_timer_queue_count_ += 1;
    MaybeCompactTimerQueue();
    return true;
}
...
void VmInstance::ReleaseTimerHandles(const std::shared_ptr<TimerTask>& task) {
    if (!task) {
        return;
    }
...
void VmInstance::MaybeCompactTimerQueue() {
    if (stale_timer_queue_count_ == 0) {
        return;
    }

    if (timers_by_id_.empty()) {
        CompactTimerQueue();
        return;
    }

    if (stale_timer_queue_count_ < 8) {
        return;
    }

    if (stale_timer_queue_count_ < timers_by_id_.size()) {
        return;
    }
...
void VmInstance::CompactTimerQueue() {
    TimerQueue compacted;
    for (const auto& entry : timers_by_id_) {
    ...
    timer_queue_ = std::move(compacted);
    stale_timer_queue_count_ = 0;
}
...
void VmInstance::RunLoopOnce(std::chrono::milliseconds max_duration) {
    ...
    if (!PostTask([this, max_duration, &done_promise]
                  (v8::Isolate* isolate, v8::Local<v8::Context> context) {
...
VmRuntimeStats VmInstance::GetRuntimeStats(bool force_gc) {
    ...
    if (!PostTask([this, force_gc, &stats, &done_promise](v8::Isolate* isolate, v8::Local<v8::Context> context) {
        (void)context;
        v8::HandleScope handle_scope(isolate);
        ...
        stats.stale_timer_queue_count = stale_timer_queue_count_;
        ...
        RefreshPlatformRuntimeStats(stats);
...
void VmInstance::RegisterWithPlatform() {
    if (!isolate_) {
        return;
    }
    V8Platform::Instance().RegisterIsolate(isolate_, this);
}

void VmInstance::UnregisterFromPlatform() {
    if (!isolate_) {
        return;
    }
    V8Platform::Instance().UnregisterIsolate(isolate_);
}

size_t VmInstance::DrainPlatformTasks(v8::platform::MessageLoopBehavior behavior,
                                      size_t max_iterations) {
    if (!isolate_) {
        return 0;
    }
    return V8Platform::Instance().DrainMessageLoop(isolate_, behavior, max_iterations);
}

void VmInstance::RefreshPlatformRuntimeStats(VmRuntimeStats& stats) const {
    if (!isolate_) {
        return;
    }
    const auto platform_stats = V8Platform::Instance().GetIsolateStats(isolate_);
    stats.platform_pending_foreground_tasks = platform_stats.pending_foreground_tasks;
    stats.platform_pending_background_tasks = platform_stats.pending_background_tasks;
    stats.platform_delayed_task_count = platform_stats.delayed_task_count;
    stats.platform_pump_count = platform_stats.pump_count;
    stats.platform_pump_iterations = platform_stats.pump_iterations;
    stats.platform_average_foreground_wait_ms =
        platform_stats.average_foreground_wait_ms;
    stats.platform_average_background_wait_ms =
        platform_stats.average_background_wait_ms;
    stats.platform_last_drain_ms = platform_stats.last_drain_ms;
    stats.platform_overload_score = platform_stats.overload_score;
}

bool VmInstance::QuiescePlatformForShutdown(std::chrono::milliseconds timeout,
                                            size_t max_iterations_per_drain,
                                            size_t consecutive_idle_rounds) {
    if (!isolate_) {
        return true;
    }
    ...
    while (std::chrono::steady_clock::now() <= deadline) {
        const size_t drained = DrainPlatformTasks(
            v8::platform::MessageLoopBehavior::kDoNotWait,
            max_iterations_per_drain);
        total_drained += drained;

        isolate_->PerformMicrotaskCheckpoint();
        const auto snapshot = V8Platform::Instance().GetIsolateStats(isolate_);
        const bool idle =
            drained == 0 && snapshot.pending_foreground_tasks == 0;

        if (idle) {
            idle_rounds += 1;
            if (idle_rounds >= consecutive_idle_rounds) {
                LEAPVM_LOG_INFO(
                    "[shutdown] platform quiesced drained=%zu idle_rounds=%zu",
                    total_drained,
                    idle_rounds);
                return true;
            }
    ...
    const auto snapshot = V8Platform::Instance().GetIsolateStats(isolate_);
    LEAPVM_LOG_WARN(
        "[shutdown] platform quiesce timeout drained=%zu pending_fg=%zu pending_bg=%zu delayed=%zu last_drain_ms=%.3f overload=%.3f",
        total_drained,
        snapshot.pending_foreground_tasks,
        snapshot.pending_background_tasks,
        snapshot.delayed_task_count,
        snapshot.last_drain_ms,
        snapshot.overload_score);
    return false;
}
...
void VmInstance::ThreadMain() {
    // VM 线程主循环：处理任务队列
    // 所有 V8 操作都在这个线程中执行
    const size_t current_thread_hash = CurrentThreadHash();
    vm_thread_hash_.store(current_thread_hash, std::memory_order_release);
    if (ShouldTraceVmThreadAffinity()) {
        std::fprintf(stderr,
                     "[leapvm][vm] thread-main vm=%p vm_thread_tid=%zu creator_tid=%zu\n",
                     static_cast<void*>(this),
                     current_thread_hash,
                     creator_thread_hash_);
        std::fflush(stderr);
    ...
    // 泵送后备平台队列，清理初始化阶段遗留的前台任务。
    DrainPlatformTasks(v8::platform::MessageLoopBehavior::kDoNotWait);
...
bool VmInstance::PostTask(Task task) {
    std::lock_guard<std::mutex> lock(task_mu_);
    if (!task || !vm_thread_running_ || !accepting_tasks_) {
        return false;
    }
...
bool VmInstance::WaitForAndProcessOneTask(std::chrono::milliseconds timeout) {
    ...
    // 先处理后备平台队列，避免 Inspector 留下的前台任务饥饿。
    DrainPlatformTasks(v8::platform::MessageLoopBehavior::kDoNotWait);
    ...
    Task task;
    {
        ...
        // 等待任务到达或超时
        if (task_queue_.empty()) {
            if (task_cv_.wait_for(lock, timeout, [this] {
                    return !task_queue_.empty() || !vm_thread_running_ || !accepting_tasks_;
                })) {
                // 有任务到达
            } else {
                // 超时
...
bool VmInstance::InitInspector(int port, const std::string& target_id) {
    ...
    if (!PostTask([this, &done](v8::Isolate* isolate, v8::Local<v8::Context> context) {
        inspector_client_->Initialize(context);
        done.set_value();
    })) {
        inspector_client_.reset();
        LEAPVM_LOG_ERROR("[inspector] Failed to schedule inspector initialization");
        return false;
    }
