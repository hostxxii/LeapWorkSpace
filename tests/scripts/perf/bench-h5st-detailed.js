...
function summarizePlatformStats(poolStats) {
  const platform = poolStats && poolStats.platform && typeof poolStats.platform === 'object'
    ? poolStats.platform
    : {};
  const workersDetail = Array.isArray(poolStats && poolStats.workersDetail)
    ? poolStats.workersDetail
    : [];
  const topWorkers = workersDetail
    .filter((worker) => worker && worker.platformStats)
    .sort((left, right) => {
      const leftScore = Number(left.platformStats && left.platformStats.overloadScore || 0);
      const rightScore = Number(right.platformStats && right.platformStats.overloadScore || 0);
    ...
    .map((worker) => ({
      ...
      overloadScore: Number(worker.platformStats && worker.platformStats.overloadScore || 0),
      pendingForegroundTasks: Number(worker.platformStats && worker.platformStats.pendingForegroundTasks || 0),
      pendingBackgroundTasks: Number(worker.platformStats && worker.platformStats.pendingBackgroundTasks || 0),
      delayedTaskCount: Number(worker.platformStats && worker.platformStats.delayedTaskCount || 0),
      lastDrainMs: Number(worker.platformStats && worker.platformStats.lastDrainMs || 0)
    }));

  return {
    pendingForegroundTasks: Number(platform.pendingForegroundTasks || 0),
    pendingBackgroundTasks: Number(platform.pendingBackgroundTasks || 0),
    delayedTaskCount: Number(platform.delayedTaskCount || 0),
    maxOverloadScore: Number(platform.maxOverloadScore || 0),
    maxDrainMs: Number(platform.maxDrainMs || 0),
    topWorkers
  };
}
...
async function runCase(mode, PoolCtor, config, targetScript) {
  ...
  return {
    ...
    platform: summarizePlatformStats(endStats),
    poolStats: endStats
  };
}
