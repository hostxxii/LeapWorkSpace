...
async function testCanvasCore() {
  const result = await runTaskJson('canvas-core', `
      (function () {
...
async function testCanvasProfileOverrideAndReset() {
  const custom = 'data:image/png;base64,TEVBUF9DQU5WQVNfUFJPRklMRQ==';
  const a = await runTaskJson('canvas-profile-a', `
      (function () {
        var c = document.createElement('canvas');
        c.width = 320;
        c.height = 200;
  ...
  const b = await runTaskJson('canvas-profile-b', `
      (function () {
        var c = document.createElement('canvas');
        var gl = c.getContext('webgl');
        return JSON.stringify({
...
(async () => {
  await testCanvasCore();
  await testCanvasProfileOverrideAndReset();
