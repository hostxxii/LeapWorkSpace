...
async function runTaskJson(taskId, targetScript) {
  return runJsonWithPreloadedTarget(targetScript, { taskId }, { debug: false }, taskId);
}

async function main() {
  const result = await runTaskJson('branded-collections', `
      (function () {
