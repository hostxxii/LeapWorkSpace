...
function runProbe(leapvm, taskId, fingerprintSnapshot, markerValue) {
  ...
  return parseJson(result, taskId);
}

function runDefaultProbe(leapvm, taskId) {
  const result = executeSignatureTask(leapvm, {
    taskId,
    targetScript: `
      (function () {
...
function main() {
  ...
  try {
    ...
    const a = runProbe(ctx.leapvm, 'fp-snapshot-a', snapshotA, 'A');
    ...
    const b = runProbe(ctx.leapvm, 'fp-snapshot-b', snapshotB, 'B');
    ...
    const d = runDefaultProbe(ctx.leapvm, 'fp-snapshot-default');
