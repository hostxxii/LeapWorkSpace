...
async function main() {
  // DOM/BOM/Event core
  const core = await runJsonWithPreloadedTarget(`
        (function () {
    ...
    if (core.documentAll.apiPath.exists) {
  ...
  // Timer API surface
  // Standalone tasks run in isolated execution units; async timer callbacks
  // cannot be verified across tasks. Verify API surface and ID semantics.
  const timer = await runJsonWithPreloadedTarget(`
        (function () {
          var probe = {};

          // setTimeout returns numeric ID > 0
          var timeoutId = setTimeout(function () {}, 10);
          probe.timeoutIdType = typeof timeoutId;
          probe.timeoutId = Number(timeoutId) || 0;
          clearTimeout(timeoutId);

          // setInterval returns numeric ID > 0
          var intervalId = setInterval(function () {}, 10);
          probe.intervalIdType = typeof intervalId;
          probe.intervalId = Number(intervalId) || 0;
          clearInterval(intervalId);

          // requestAnimationFrame returns numeric ID > 0
          var rafId = requestAnimationFrame(function (ts) {});
          probe.rafIdType = typeof rafId;
          probe.rafId = Number(rafId) || 0;

          // cancelAnimationFrame should not throw
          var canceledId = requestAnimationFrame(function () {});
          probe.canceledRafIdType = typeof canceledId;
          probe.canceledRafId = Number(canceledId) || 0;
          cancelAnimationFrame(canceledId);
          probe.cancelDidNotThrow = true;

          // Timer IDs should be distinct
          var id1 = setTimeout(function () {}, 1);
          var id2 = setTimeout(function () {}, 1);
          probe.idsDistinct = id1 !== id2;
          clearTimeout(id1);
          clearTimeout(id2);
      ...
      `,
      {
      taskId: 'signature-timer-probe',
      },
      { debug: false },
      'signature-timer-probe'
    );
    ...
    assert.strictEqual(timer.cancelDidNotThrow, true);
    assert.strictEqual(timer.idsDistinct, true);
