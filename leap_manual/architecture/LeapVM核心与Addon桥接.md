...
> 源文件：`leap-vm/src/leapvm/vm_instance.cc`，`leap-vm/src/leapvm/vm_instance.h`，`leap-vm/src/leapvm/v8_platform.cc`，`leap-vm/src/leapvm/v8_platform.h`，`leap-vm/src/addon/main.cc`，`leap-vm/src/addon/vm_instance_wrapper.cc`，`leap-vm/src/addon/vm_instance_wrapper.h`，`leap-vm/index.js`
> 更新：2026-03-05
...
### 0. Node 侧 addon 加载策略（Linux worker）

`leap-vm/index.js` 负责定位并加载 `leapvm.node`。在 Linux `worker_threads` 场景下，默认采用：

- `process.dlopen(..., RTLD_NOW | RTLD_NODELETE)`

核心目的：避免 worker 退出时 `dlclose(leapvm.node)` 后，线程 TLS 清理仍跳回 addon 地址导致的 `SIGSEGV`。  
该加载策略属于 Node 层防护，和 addon 内部 NAPI 生命周期管理互补。
...
```
JS: leapenv.getNativeBridge().defineEnvironmentSkeleton(skeletonDef)
...
```
JS: leapenv.getNativeBridge().createChildFrame(url, sameOrigin)
