...
- **Impl**（行为层）：JS 文件通过 runtime bridge dispatch 路由到对应 Impl；
- **C++ 组装层**：`SkeletonRegistry` 读取 Skeleton 描述，用 V8 FunctionTemplate 构建原型链，所有 method/accessor stub 都通过 `DispatchBridge::StubCallback` 路由到 `leapenv.__runtime.bridge.dispatch`。

Dispatch 系统是 Skeleton 的运行时路由层：所有 stub 触发后统一调用 JS dispatch 函数 `leapenv.__runtime.bridge.dispatch(typeName, propName, actionType, ...args)`，在 JS impl 注册表中查找并执行对应实现。
...
leapenv.loadSkeleton()
  ...
  └── leapenv.getNativeBridge().defineEnvironmentSkeleton(envDescriptor)  // 调用 C++ API
```
...
→ DispatchBridge::StubCallback(args)
  ...
  ├── GetDispatchFn()：从 VmInstance 缓存取 dispatch_fn（首次从 leapenv.__runtime.bridge.dispatch 查找并缓存）
  ├── 组装 dispatch_args = [typeName, propName, "GET"|"SET"|"CALL", ...原始args]
  └── dispatch_fn.call(this, typeName, propName, actionType, ...args)
        → JS impl 层查找并执行
```
...
子帧创建时，C++ 在新 Context 中重新执行 `bundle_source_`，由 `NativeDefineEnvironmentSkeleton` 检测到子上下文后**独立构建** `SkeletonRegistry`，保证子帧拥有与主帧相同的类型定义。dispatch 函数缓存存储在 `VmInstance` 中（每个 context 各自一份）。
...
2. JS: 执行 leap-env bundle (leapenv.js)
   a. runtime.js: 注册 dispatch 到 leapenv.__runtime.bridge.dispatch
   ...
   d. skeleton-loader.js: leapenv.loadSkeleton()
      ...
      → leapenv.getNativeBridge().defineEnvironmentSkeleton(envDescriptor)
...
→ GetDispatchFn() → leapenv.__runtime.bridge.dispatch（缓存命中）
→ dispatch_fn.call(navigator, "Navigator", "userAgent", "GET")
